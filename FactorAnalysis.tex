...
In particular the ideas of factor investment presented by Fama \& French in 1993. \textcolor{red}{include reference}
This section will perform a analysis similar to the one by Fama \& French \textcolor{red}{(include reference)} to investigate the relevancy of the factors in a modern and European market.

The market factor describes how the overall market returns
affects the returns of individual assets. The momentum factor captures how current high-performing assets will perform in the future. 
While the size factor attempts to describe how the size of a company affects its returns.
In the original analysis by Fama \& French \textcolor{red}{(include reference)} it is observed that smaller companies tend to outperform larger companies.\newline

The analysis will be based on returns from American and European Markets in the period from from September 2004 to December 2024.
It should be noted that this period is affected by both several major market events and the period of \textit{Quantitative Easening}.
Possibly leading to different conclusions than had the analysis been performed on a different time period.

In order for analysis to fulfill the needs of Best Pension the analysis method from Fama \& French \textcolor{red}{(include reference)} will be performed in the following scenarios
\begin{itemize}
...
The results of our analysis indicate that the three factors are indeed still relevant in all three scenarios. 
As Best Pensions is a EUR-based investor that can potentially invest in both US and European equities, the second and third analysis highlights that it could indeed be suitable for Best Pensions to consider these factors in their asset allocation.
There is however the caveat that Best Pensions currently doesn't allow short and as such investing in MOM and SMB portfolios is not allowed.
\newline
...
\input{R/Output/USD_Based_US_R2_format.tex}
\newline
...
\input{R/Output/EUR_R2_format.tex}
...
\input{R/Output/factors_EUR_US_colored_format.tex}
