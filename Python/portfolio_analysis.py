...
test = spot_rates.with_columns(Zero_cupon_price = ( - (pl.col("TIME_TO_MATURITY") / 12) * (pl.col("SPOTRATE") / 100)).exp()).filter(pl.col("TIME_TO_MATURITY") == 120)

px.line(test, x = "TIME_PERIOD", y = "Zero_cupon_price").show()
