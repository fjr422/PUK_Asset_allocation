...
class ActiveReserveStrategy:
    ...
    def cppi_strategies(self, m: float, b = 1, l_target = common_var.l_target, initial_contribution = 100):
        """CPPI strategy.
        Specify
        ...
        for date, portfolio in tdf_runs.items():
            ...
            values_strategy = self.apply_investment.portfolio_values("Tie in for " + tdf_strategy.value, initial_weights_cppi, initial_values_cppi, self.__cppi_strategy(self.active_strategy, tdf_strategy, m = m, b=b), start_period = cppi_start_pl, start_period_pd = cppi_start_pd, end_period = cppi_end_pl)
            portfolio_values_df.extend(values_strategy)
